  ...
  return(
      ...
      <div className={creating ? "creationContainer" : "hidden"}>
          ...
          <form className="campaignCreateOptions" onSubmit={createCampaign}>
              ...
              <div className="campaignSection">
                <h1>Campaign Name</h1>
                <input type="text" className="campaignNameTextBox" placeholder="Your Story Begins Here!" onChange={(e)=>(setCampaignName(e.target.value))}/>
              </div>
              <div className="campaignSection">
                <h1>{"Description"}</h1>
                <input type="text" className="campaignNameTextBox" placeholder="Let viewers get a rough idea of your story!"  onChange={(e)=>(setDescription(e.target.value))}/>
              </div>
              <div className="campaignSection">
                <h1>{"Privacy"}</h1>
              ...
              <input type="button" className="campaignNameClose" value="Cancel" onClick={toggleCampaignCreateOptions}/>
      ...
      <div className={!creating ? "mainContainer" : "hidden"}>
        <div className="leftContainer">
            <button className="createButton" onClick={toggleCampaignCreateOptions}>Create</button>
        </div>

        <div className="centerContainer">
          {campaigns.map((campaign, index)=>(
            <div key={index} data-name={campaign.campaignName} className="campaignContainer" onClick={visitCampaign}>
              <h2>{campaign.campaignName}</h2>
              <h2 key={campaign.campaignName} className="deleteButton" onClick={deleteCampaign}> Delete</h2>
            </div>
          ))}
        </div>
